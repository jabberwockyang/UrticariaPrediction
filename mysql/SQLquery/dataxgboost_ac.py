...
def modify_sql(sql_template, itemlistlist, classifications):
    ...
    AVG(CASE WHEN npe.ExaminationItemID = '{item_ID}' AND npe.ExaminationDate > DATE(p.FirstVisitDate, '+42 days') THEN npe.Result ELSE NULL END) AS {item_english}_Avg_chronic,
    MAX(CASE WHEN npe.ExaminationItemID = '{item_ID}' AND npe.ExaminationDate > DATE(p.FirstVisitDate, '+42 days') THEN npe.Result ELSE NULL END) AS {item_english}_Max_chronic,
    MIN(CASE WHEN npe.ExaminationItemID = '{item_ID}' AND npe.ExaminationDate > DATE(p.FirstVisitDate, '+42 days') THEN npe.Result ELSE NULL END) AS {item_english}_Min_chronic"""
...
with open(f'sql/dataforxgboost_ac_{current_date}.sql', 'w') as f:
